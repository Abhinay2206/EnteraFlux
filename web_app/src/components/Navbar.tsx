...
export default function Navbar() {
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'py-2' : 'py-4'}`}>
            ...
            <div className={`absolute inset-0 transition-all duration-500 pointer-events-none ${scrolled
            ...
            <nav className={`relative max-w-[1200px] mx-4 lg:mx-auto rounded-2xl transition-all duration-500 ${scrolled
                ...
                <div className="px-5 sm:px-6 lg:px-8">
                        ...
                        <div className="hidden lg:flex items-center gap-1">
                            {navigation.map((item) => (
                                <Link
                                    key={item.name}
                                    to={item.href}
                                    className={`relative px-4 py-2 rounded-lg text-[13.5px] font-medium transition-all duration-200 ${location.pathname === item.href
                    ...
                    {/* Mobile menu */}
                    <div className={`lg:hidden transition-all duration-300 ease-out overflow-hidden ${mobileMenuOpen ? 'max-h-[400px] opacity-100 pb-5' : 'max-h-0 opacity-0'
                        }`}>
                        <div className="pt-3 space-y-1 border-t border-neutral-200/50 dark:border-white/[0.06]">
                            <Link
                                to="/"
                                className={`block px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${location.pathname === '/'
                            ...
                            {navigation.map((item) => (
                                <Link
                                    key={item.name}
                                    to={item.href}
                                    className={`block px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${location.pathname === item.href
                                            ? 'bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-400'
                                            : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-white/[0.04]'
                                        }`}
                                    onClick={() => setMobileMenuOpen(false)}
